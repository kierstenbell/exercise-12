...
- For each of the two response variables, indicate what is the best model overall based on AICc and how many models have a delta AICc of 4 or less.
    ...
    - See below. For `max. life(mo)` m4 is the best model which includes log transformed variables of `gestation(mo)`, `litters/year`, `mass(g)`,`weaning(mo)`. It is the only model that has a delta AICc of 4 or less. For `AFR(m0)`, lm4 is the best model which includes log transformed variables of `litters/year`, `gestation(mo)`, `wean mass(g)`, and `weaning(mo)`. It is the only model that has  a delta AICc of 4 or less. 
    
- What variables, if any, appear in all of this set of “top” models?

    - The variables that appear in all the sets are log transformed `gestation(mo)`, `litters/year`, and `weaning(mo)`. 
