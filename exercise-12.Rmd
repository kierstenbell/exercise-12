...
library(MuMIn)
# Max life (mo)
# Using information from M4 with na.action = "na.fail"
m.MaxLife <- lm(data = d_maxLife, `max. life(mo)` ~ `gestation(mo)` + `litters/year` + `mass(g)` + `weaning(mo)`, na.action = "na.fail")

#mods.MaxLife <- dredge(m.MaxLife, beta = "none", m.lim = c(0, 4))
#mods.avg.MaxLife <- summary(model.avg(mods.MaxLife, subset = delta < 4, fit = TRUE))
#confint(mods.avg.MaxLife)
